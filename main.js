...
function setupNfc() {
    ...
    enrollNfcButton.addEventListener('click', async () => {
        try {
            ...
            if (!user) {
                nfcStatus.textContent = 'You must be logged in to enroll a tag';
                return;
            }
            
            // Generate a custom token for NFC login
            ...
            nfcStatus.textContent = 'Tag enrolled successfully!';
        } catch (error) {
...
// Initialize NFC when DOM is loaded
...
function setupDirectMessaging() {
            ...
            snapshot.forEach((userSnapshot) => {
                ...
                if (user.uid !== firebase.auth().currentUser.uid) {
                    ...
                    userElement.addEventListener('click', () => {
                        openDMConversation(user.uid, user.displayName || user.email);
                    });
                    dmList.appendChild(userElement);
                    userCount++;
                }
            });
            if (userCount === 0) {
                const noUsersMsg = document.createElement('div');
                noUsersMsg.className = 'dm-no-users';
                noUsersMsg.textContent = 'No users available for direct messages.';
                dmList.appendChild(noUsersMsg);
            }
        });
    }

    // Open DM conversation
    function openDMConversation(recipientUid, recipientName) {
        currentDmRecipientUid = recipientUid;
        currentDmRecipientName = recipientName;
        dmRecipientName.textContent = recipientName;
    ...
    // Load users when DM screen is opened
    document.getElementById('direct-messages-button').addEventListener('click', loadDMUsers);
}

// Initialize direct messaging when DOM is loaded
document.addEventListener('DOMContentLoaded', setupDirectMessaging);
